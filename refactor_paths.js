...
function processFile(filepath) {
    ...
    // Admin checking 'acw_attempts' across users
    // If AdminAcwStats, AdminAgents, AdminResults, ExecutiveReportPage
    if (filepath.includes('Admin') || filepath.includes('Executive')) {
        ...
        // deleteDoc for resultados_quizzes (requires reference to specific user? wait, doc() won't work easily if we don't know the user id)
        // Wait, if an admin uses deleteDoc(doc(db, 'resultados_quizzes', id)), this is broken if they don't have userId!
        // We will leave the regular doc replacement out for a moment and handle it manually by checking if deleteDoc is used.
    }
